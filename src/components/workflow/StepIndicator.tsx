...
export const StepIndicator: React.FC<StepIndicatorProps> = memo(
  ({ steps, currentStepIndex, onStepClick }) => {
    ...
    const progressPercentage =
      steps.length > 0 ? (currentStepIndex / (steps.length - 1)) * 100 : 0;
        ...
        <div className="flex justify-between items-center">
          {steps.map((step, index) => {
            ...
            const isWaitingPermission =
              step.status === StepStatus.WAITING_PERMISSION;
            ...
            return (
              <div key={step.id} className="flex flex-col items-center flex-1">
                ...
                {index < steps.length - 1 && (
                  <div
                    className={`${lineClasses} ${index < currentStepIndex ? "bg-green-500" : "bg-gray-300"}`}
                  ></div>
                )}
              </div>
            );
